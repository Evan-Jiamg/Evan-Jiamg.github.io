...
\begin{bullets}
  \item 這篇發表在 International Conference on Computational Science and Network Intelligence 的論文，是我在大學專題期間擔任第一作者的研究成果。該會議主要聚焦在結合網路分析、圖神經網路和人工智慧，來解決真實世界問題的國際會議。我在這篇論文中提出的核心創新，是將傳統演化賽局的 Friedkin-Johnsen Model 與具備語言推理能力的 LLM Agent 併入同張 Opinion Game 動態重連的演化網路，填補過去 Opinion Game 缺乏語言推理，以及 LLM Simulation 未衡量 Social Efficiency 的缺口，並結合實證分析，利用真實 Reddit 數據做為 Agent 立場，經過 540 次 Simulations，發現 LLM Agent 之間最容易達成共識，但這個共識的代價是整體社會成本最高，純 LLM Agent 網路的效率損失為傳統 FJ Model 的五倍，而同溫層的形成則取決於動態重連機制，而非初始網路結構。
\end{bullets}
